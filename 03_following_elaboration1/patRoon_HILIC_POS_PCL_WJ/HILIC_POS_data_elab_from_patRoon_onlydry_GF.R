...
HLPnrf <- read_csv("featureGroupsXCMS3_updt.csv") %>%
...
HLPnf <- select(HLPnrf, all_of(c("group", "ret", "mz", analyses_dry_samples)))

all_featuresHLPnf <- get_noNAs_features(HLPnf)

HLP <- filter(HLPnf, group %in% all_featuresHLPnf)
...
HLP0 <- filter(HLP0nf, group %in% all_featuresHLP0nf)

write_tsv(HLP0, "HLP0.txt")

HLP_feat_info <- select(HLP0, group, ret, mz)
...
HLP_unique_feat_vector <- unique(c(HLP0_dry_N2_ACNf$group, HLP0_dry_VC40_ACNf$group, HLP0_dry_VC1668_ACNf$group, HLP0_dry_BR5270_ACNf$group, HLP0_dry_UA57_ACNf$group, HLP0_dry_VC40_ACN_bisf$group,
                                   HLP0_dry_N2_2Bf$group, HLP0_dry_VC40_2Bf$group, HLP0_dry_VC1668_2Bf$group, HLP0_dry_BR5270_2Bf$group, HLP0_dry_UA57_2Bf$group, HLP0_dry_VC40_2B_bisf$group))


HLP1 <- filter(HLP0, group %in% HLP_unique_feat_vector) %>%
  select(-ret, -mz)
...
HLP_unique_feat_vector_dry_schem1 <- unique(c(HLP0_dry_N2_ACNf$group, HLP0_dry_VC40_ACNf$group, HLP0_dry_VC1668_ACNf$group, HLP0_dry_BR5270_ACNf$group, HLP0_dry_UA57_ACNf$group, HLP0_dry_VC40_ACN_bisf$group))

HLP_unique_feat_vector_dry_schem2 <- unique(c(HLP0_dry_N2_2Bf$group, HLP0_dry_VC40_2Bf$group, HLP0_dry_VC1668_2Bf$group, HLP0_dry_BR5270_2Bf$group, HLP0_dry_UA57_2Bf$group, HLP0_dry_VC40_2B_bisf$group))
...
plot(euler(HLP_EV_df[, 2:3], shape = "ellipse"), quantities = TRUE)
dev.off()
...
jpeg(file="HILIC_POS_EuleroVenn_bis_dry.jpeg")
...
write_tsv(HLP4_shem1_dry_transf, "HLP4_shem1_dry_transf.txt")
write_tsv(HLP4_shem1_dry_ANOVA, "HLP4_shem1_dry_ANOVA.txt")

write_tsv(HLP4_shem2_dry_transf, "HLP4_shem2_dry_transf.txt")
write_tsv(HLP4_shem2_dry_ANOVA, "HLP4_shem2_dry_ANOVA.txt")
...
write_tsv(HLP4_shem1_dry_ANOVA_sign, "HLP4_shem1_dry_ANOVA_sign.txt")
write_tsv(HLP4_shem2_dry_ANOVA_sign, "HLP4_shem2_dry_ANOVA_sign.txt")
...
MFsummary_PCL <- read_tsv("MFsummary_PCL_no_formula_updt.txt")


MFsummary_PCL_colnames <- setNames(rep("", length(colnames(MFsummary_PCL))), colnames(MFsummary_PCL))
MFsummary_PCL_empty <- bind_rows(MFsummary_PCL_colnames)[0, ]
...
MFsummary_PCL_empty_for_HLP4_shem1_dry_ANOVA_sign_annotation <- MFsummary_PCL_empty
MFsummary_PCL_empty_for_HLP4_shem1_dry_ANOVA_sign_annotation[1:length(HLP4_shem1_dry_ANOVA_sign$feature),] <- NA

HLP4_shem1_dry_ANOVA_sign_annotation <- cbind(HLP4_shem1_dry_ANOVA_sign, MFsummary_PCL_empty_for_HLP4_shem1_dry_ANOVA_sign_annotation)

for (s in 1:length(HLP4_shem1_dry_ANOVA_sign_annotation$feature)) {
  if (HLP4_shem1_dry_ANOVA_sign_annotation$feature[s] %in% MFsummary_PCL$group) {
    HLP4_shem1_dry_ANOVA_sign_annotation[s,7:39] <- filter(MFsummary_PCL, group == HLP4_shem1_dry_ANOVA_sign_annotation$feature[s])[1,]
  }
}

write_tsv(HLP4_shem1_dry_ANOVA_sign_annotation, "HLP4_shem1_dry_ANOVA_sign_annotation.txt")




MFsummary_PCL_empty_for_HLP4_shem2_dry_ANOVA_sign_annotation <- MFsummary_PCL_empty
MFsummary_PCL_empty_for_HLP4_shem2_dry_ANOVA_sign_annotation[1:length(HLP4_shem2_dry_ANOVA_sign$feature),] <- NA

HLP4_shem2_dry_ANOVA_sign_annotation <- cbind(HLP4_shem2_dry_ANOVA_sign, MFsummary_PCL_empty_for_HLP4_shem2_dry_ANOVA_sign_annotation)

for (s in 1:length(HLP4_shem2_dry_ANOVA_sign_annotation$feature)) {
  if (HLP4_shem2_dry_ANOVA_sign_annotation$feature[s] %in% MFsummary_PCL$group) {
    HLP4_shem2_dry_ANOVA_sign_annotation[s,7:39] <- filter(MFsummary_PCL, group == HLP4_shem2_dry_ANOVA_sign_annotation$feature[s])[1,]
  }
}

write_tsv(HLP4_shem2_dry_ANOVA_sign_annotation, "HLP4_shem2_dry_ANOVA_sign_annotation.txt")
...
MFsummary_PCL_topMONA_empty <- MFsummary_PCL[0,]
colnames(MFsummary_PCL_topMONA_empty) <- paste0(colnames(MFsummary_PCL), "_topMONA")
MFsummary_PCL_topMONA_empty[1:length(HLP_feat_info$group),] <- NA

MFsummary_PCL_topScore_empty <- MFsummary_PCL[0,]
colnames(MFsummary_PCL_topScore_empty) <- paste0(colnames(MFsummary_PCL), "_topScore")
MFsummary_PCL_topScore_empty[1:length(HLP_feat_info$group),] <- NA


HLP_feat_info_annotation <- cbind(mutate(HLP_feat_info, AnnoLevel = as.character(NA), is_topMONA_topScore = as.logical(NA)), MFsummary_PCL_topMONA_empty, MFsummary_PCL_topScore_empty)


for (i in 1:length(HLP_feat_info$group)) {
  if (HLP_feat_info$group[i] %in% MFsummary_PCL$group) {
    MFsummary_PCL_fil <- filter(MFsummary_PCL, group == HLP_feat_info$group[i])
    topMONA_row <- which.max(MFsummary_PCL_fil$individualMoNAScore)
    topScore_row <- which.max(MFsummary_PCL_fil$score)
    
    HLP_feat_info_annotation[i,paste0(colnames(MFsummary_PCL), "_topMONA")] <- MFsummary_PCL_fil[topMONA_row,colnames(MFsummary_PCL_fil)]
    HLP_feat_info_annotation[i,paste0(colnames(MFsummary_PCL), "_topScore")] <- MFsummary_PCL_fil[topScore_row,colnames(MFsummary_PCL_fil)]
    
    HLP_feat_info_annotation[i, "is_topMONA_topScore"] <- HLP_feat_info_annotation[i, "identifier_topMONA"] == HLP_feat_info_annotation[i, "identifier_topScore"]
    
    HLP_feat_info_annotation[i, "AnnoLevel"] <- ifelse(HLP_feat_info_annotation[i,"individualMoNAScore_topMONA"]>0.9, "2a",
                                                       ifelse(HLP_feat_info_annotation[i,"individualMoNAScore_topMONA"]<0.9 & HLP_feat_info_annotation[i,"individualMoNAScore_topMONA"]>0.7, "3a",
                                                              ifelse(HLP_feat_info_annotation[i,"individualMoNAScore_topMONA"]<0.7 & HLP_feat_info_annotation[i,"individualMoNAScore_topMONA"]>0.4, "3b",
                                                                     ifelse(mean(MFsummary_PCL_fil$neutral_formula == MFsummary_PCL_fil$neutral_formula[1]) == 1, "4a", "5"))))
    
  } else {
    HLP_feat_info_annotation[i, "AnnoLevel"] <- "5"
  }
}

write_tsv(HLP_feat_info_annotation, "HILICPOS_patRoon_IPO_PCL_feat_info_annotation.txt")


## annotation levels, after filtering QCs:

HLP_feat_info_annotation_QCfiltered <- filter(HLP_feat_info_annotation, group %in% HLP_unique_feat_vector) 

write_tsv(HLP_feat_info_annotation_QCfiltered, "HILICPOS_patRoon_IPO_PCL_feat_info_annotation_QCfiltered.txt")


## annotation levels, after filtering QCs, per sample scheme:

HLP_feat_info_annotation_QCfiltered_scheme1_dry <- filter(HLP_feat_info_annotation, group %in% HLP_unique_feat_vector_dry_schem1) 

write_tsv(HLP_feat_info_annotation_QCfiltered_scheme1_dry, "HILICPOS_patRoon_IPO_PCL_feat_info_annotation_QCfiltered_scheme1_dry.txt")


HLP_feat_info_annotation_QCfiltered_scheme2_dry <- filter(HLP_feat_info_annotation, group %in% HLP_unique_feat_vector_dry_schem2) 

write_tsv(HLP_feat_info_annotation_QCfiltered_scheme2_dry, "HILICPOS_patRoon_IPO_PCL_feat_info_annotation_QCfiltered_scheme2_dry.txt")
...
HLP_feat_info_annotation_empty_dried_scheme1 <- HLP_feat_info_annotation[0,]
HLP_feat_info_annotation_empty_dried_scheme1[1:length(HLP4_shem1_dry_ANOVA_sign$feature),] <- NA


HLP4_shem1_dry_ANOVA_sign_annotat_withLevels <- cbind(HLP4_shem1_dry_ANOVA_sign, HLP_feat_info_annotation_empty_dried_scheme1)

for (i in 1:length(HLP4_shem1_dry_ANOVA_sign$feature)) {
  indices <- which(HLP_feat_info_annotation$group == HLP4_shem1_dry_ANOVA_sign$feature[i])
  
  HLP4_shem1_dry_ANOVA_sign_annotat_withLevels[i,colnames(HLP_feat_info_annotation_empty_dried_scheme1)] <- HLP_feat_info_annotation[indices,]
  
}

write_tsv(HLP4_shem1_dry_ANOVA_sign_annotat_withLevels, "HILICPOS_patRoon_IPO_PCL_scheme1_dry_ANOVA_sign_annotat_withLevels.txt")




HLP_feat_info_annotation_empty_dried_scheme2bottom <- HLP_feat_info_annotation[0,]
HLP_feat_info_annotation_empty_dried_scheme2bottom[1:length(HLP4_shem2_dry_ANOVA_sign$feature),] <- NA


HLP4_shem2_dry_ANOVA_sign_annotat_withLevels <- cbind(HLP4_shem2_dry_ANOVA_sign, HLP_feat_info_annotation_empty_dried_scheme2bottom)

for (i in 1:length(HLP4_shem2_dry_ANOVA_sign$feature)) {
  indices <- which(HLP_feat_info_annotation$group == HLP4_shem2_dry_ANOVA_sign$feature[i])
  
  HLP4_shem2_dry_ANOVA_sign_annotat_withLevels[i,colnames(HLP_feat_info_annotation_empty_dried_scheme2bottom)] <- HLP_feat_info_annotation[indices,]
  
}

write_tsv(HLP4_shem2_dry_ANOVA_sign_annotat_withLevels, "HILICPOS_patRoon_IPO_PCL_scheme2_dry_ANOVA_sign_annotat_withLevels.txt")
...
MFsummary_WJ <- read_tsv("MFsummary_WJ_no_formula_combined_bis.txt")


MFsummary_WJ_colnames <- setNames(rep("", length(colnames(MFsummary_WJ))), colnames(MFsummary_WJ))
MFsummary_WJ_empty <- bind_rows(MFsummary_WJ_colnames)[0, ]




MFsummary_WJ_empty_for_HLP4_shem1_dry_ANOVA_sign_annotation <- MFsummary_WJ_empty
MFsummary_WJ_empty_for_HLP4_shem1_dry_ANOVA_sign_annotation[1:length(HLP4_shem1_dry_ANOVA_sign$feature),] <- NA

HLP4_shem1_dry_ANOVA_sign_annotation <- cbind(HLP4_shem1_dry_ANOVA_sign, MFsummary_WJ_empty_for_HLP4_shem1_dry_ANOVA_sign_annotation)

for (s in 1:length(HLP4_shem1_dry_ANOVA_sign_annotation$feature)) {
  if (HLP4_shem1_dry_ANOVA_sign_annotation$feature[s] %in% MFsummary_WJ$group) {
    HLP4_shem1_dry_ANOVA_sign_annotation[s,7:39] <- filter(MFsummary_WJ, group == HLP4_shem1_dry_ANOVA_sign_annotation$feature[s])[1,]
  }
}
...
MFsummary_WJ_empty_for_HLP4_shem2_dry_ANOVA_sign_annotation <- MFsummary_WJ_empty
MFsummary_WJ_empty_for_HLP4_shem2_dry_ANOVA_sign_annotation[1:length(HLP4_shem2_dry_ANOVA_sign$feature),] <- NA

HLP4_shem2_dry_ANOVA_sign_annotation <- cbind(HLP4_shem2_dry_ANOVA_sign, MFsummary_WJ_empty_for_HLP4_shem2_dry_ANOVA_sign_annotation)

for (s in 1:length(HLP4_shem2_dry_ANOVA_sign_annotation$feature)) {
  if (HLP4_shem2_dry_ANOVA_sign_annotation$feature[s] %in% MFsummary_WJ$group) {
    HLP4_shem2_dry_ANOVA_sign_annotation[s,7:39] <- filter(MFsummary_WJ, group == HLP4_shem2_dry_ANOVA_sign_annotation$feature[s])[1,]
  }
}
...
MFsummary_WJ_topMONA_empty <- MFsummary_WJ[0,]
colnames(MFsummary_WJ_topMONA_empty) <- paste0(colnames(MFsummary_WJ), "_topMONA")
MFsummary_WJ_topMONA_empty[1:length(HLP_feat_info$group),] <- NA

MFsummary_WJ_topScore_empty <- MFsummary_WJ[0,]
colnames(MFsummary_WJ_topScore_empty) <- paste0(colnames(MFsummary_WJ), "_topScore")
MFsummary_WJ_topScore_empty[1:length(HLP_feat_info$group),] <- NA


HLP_feat_info_annotation <- cbind(mutate(HLP_feat_info, AnnoLevel = as.character(NA), is_topMONA_topScore = as.logical(NA)), MFsummary_WJ_topMONA_empty, MFsummary_WJ_topScore_empty)


for (i in 1:length(HLP_feat_info$group)) {
  if (HLP_feat_info$group[i] %in% MFsummary_WJ$group) {
    MFsummary_WJ_fil <- filter(MFsummary_WJ, group == HLP_feat_info$group[i])
    topMONA_row <- which.max(MFsummary_WJ_fil$individualMoNAScore)
    topScore_row <- which.max(MFsummary_WJ_fil$score)
    
    HLP_feat_info_annotation[i,paste0(colnames(MFsummary_WJ), "_topMONA")] <- MFsummary_WJ_fil[topMONA_row,colnames(MFsummary_WJ_fil)]
    HLP_feat_info_annotation[i,paste0(colnames(MFsummary_WJ), "_topScore")] <- MFsummary_WJ_fil[topScore_row,colnames(MFsummary_WJ_fil)]
    ...
    HLP_feat_info_annotation[i, "AnnoLevel"] <- ifelse(HLP_feat_info_annotation[i,"individualMoNAScore_topMONA"]>0.9, "2a",
                                                       ifelse(HLP_feat_info_annotation[i,"individualMoNAScore_topMONA"]<0.9 & HLP_feat_info_annotation[i,"individualMoNAScore_topMONA"]>0.7, "3a",
                                                              ifelse(HLP_feat_info_annotation[i,"individualMoNAScore_topMONA"]<0.7 & HLP_feat_info_annotation[i,"individualMoNAScore_topMONA"]>0.4, "3b",
                                                                     ifelse(mean(MFsummary_WJ_fil$neutral_formula == MFsummary_WJ_fil$neutral_formula[1]) == 1, "4a", "5"))))
...
write_tsv(HLP_feat_info_annotation, "HILICPOS_patRoon_IPO_WJ_feat_info_annotation.txt")
...
write_tsv(HLP_feat_info_annotation_QCfiltered, "HILICPOS_patRoon_IPO_WJ_feat_info_annotation_QCfiltered.txt")
...
write_tsv(HLP_feat_info_annotation_QCfiltered_scheme1_dry, "HILICPOS_patRoon_IPO_WJ_feat_info_annotation_QCfiltered_scheme1_dry.txt")


HLP_feat_info_annotation_QCfiltered_scheme2_dry <- filter(HLP_feat_info_annotation, group %in% HLP_unique_feat_vector_dry_schem2) 

write_tsv(HLP_feat_info_annotation_QCfiltered_scheme2_dry, "HILICPOS_patRoon_IPO_WJ_feat_info_annotation_QCfiltered_scheme2_dry.txt")
...
write_tsv(HLP4_shem1_dry_ANOVA_sign_annotat_withLevels, "HILICPOS_patRoon_IPO_WJ_scheme1_dry_ANOVA_sign_annotat_withLevels.txt")
...
write_tsv(HLP4_shem2_dry_ANOVA_sign_annotat_withLevels, "HILICPOS_patRoon_IPO_WJ_scheme2_dry_ANOVA_sign_annotat_withLevels.txt")
